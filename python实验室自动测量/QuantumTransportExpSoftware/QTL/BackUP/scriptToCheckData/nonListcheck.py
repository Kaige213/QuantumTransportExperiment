...
for n in range(len(loopState_mat)):
  ...
  if (loopState_now[0] != 0): continue
    
  #initiate
  fileName = "ID"  
  #loopID
  for m in range(len(loopState_now)-1,-1,-1):    #descending order
    ID       = loopState_now[m]
    fileName = fileName +  "." + str(ID)

  #controlState in fileName
  for m in range(len(loopState_now)-1,0,-1):
    ID       = loopState_now[m]
  ...
  #instrument with loop="1->"
  fileName = fileName + "_%s=sweep.dat"%(controlInstrument_sortedByLoop[0]["instrument"])
  
  #检查正扫数据文件名 
  pathAndFile = "%s/%s"%(folderPath,fileName)
  if (not os.path.isfile(pathAndFile)):
    print("    文件缺失: ", pathAndFile); allFileNamesAreCorrect=False
...
for n in range(len(loopState_mat)):
  ...
  #find fileName
  fileID_str = "ID."+".".join( list(map(str, loopState_now[::-1])))
  ...
  #提取回扫数据
  if (ifSweepBack == True):
    with open(folderPath+'/SweepBack/'+fileName) as f:
      for line_str in f.readlines():
        if (line_str[0] == "#"): continue
        data_vec = []
        for string in line_str.split(" "):
...
for n in range(len(loopState_mat)):
  ...
  #计算理论值
  measureState = measure_result(variable_toSave, controlState)
  #计算相对误差
  error_vec    = []
  for m in range(len(measureState)):
    if (abs(measureState[m]) > 10**-10): 
      error_vec.append((measure_mat[n][m] - measureState[m]) / abs(measureState[m]))
    else:
      error_vec.append((measure_mat[n][m] - measureState[m]))
...
#=================================================================================================================
#=================================================================================================================
#第三步：检查由figure保存的curve数据（即检查绘图是否正确）
print("检查curve数据文件：")
measure_mat = []
error_mat   = []
